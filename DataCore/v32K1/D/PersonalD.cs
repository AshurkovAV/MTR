...
namespace Medical.DataCore.v32K1.D
{
    public class PersonalD
    {
        ...
        [XmlElement(ElementName = "DOCDATE", IsNullable = false)]
        public string DocDateXml
        {
            get
            {
                return DocDate.HasValue ? DocDate.Value.ToString("yyyy-MM-dd") : null;
            }
            set
            {
                DocDate = SafeConvert.ToDateTimeExact(value.TrimNullable(), "yyyy-MM-dd");
            }
        }

        [XmlElement(ElementName = "DOCORG", IsNullable = false)]
        public string DocOrgXml
        {
            get { return DocOrg; }
            set
            {
                DocOrg = value.TrimNullable();
            }
        }
