...
{
    public class ZMedicalEventContainer : ViewModelBase, IDataErrorInfo
    {
        ...
        private Dictionary<int, ZMedicalEventModel> _cache;
        
        public List<int> MedicalEventIds { get; set; }

        public ZMedicalEventModel MedicalEvent { get; set; }
        ...
        public ZMedicalEventContainer(IMedicineRepository repository, int id, Action<int> changePosition)
        {
            _id = id;
        ...
        {
            _cache = new Dictionary<int, ZMedicalEventModel>();
            var result = _repository.GetZMedicalEventIdsByZslMeventId(_id);
            if (result.Success)
            {
        ...
        private void LoadMedicalEventData(int medicalEventId, bool forced = false)
        {
            ...
            {
                MedicalEvent = _cache[medicalEventId];
            ...
            {
                var medicalEventResult = _repository.GetZMedicalEventById(medicalEventId);
                if (medicalEventResult.Success)
                {
                    var factMedicalEvent = medicalEventResult.Data;
                    MedicalEvent = Map.ObjectToObject<ZMedicalEventModel>(factMedicalEvent, factMedicalEvent);
                    ...
                    _cache.Add(medicalEventId, MedicalEvent);
                    if (_changePosition.IsNotNull())
                    {
                        _changePosition(medicalEventId);
                    }
            ...
            RaisePropertyChanged(() => MedicalEvent);
        }
        ...
        public ICommand FirstZMedicalEventCommand
        {
        ...
        public ICommand LastZMedicalEventCommand
        {
