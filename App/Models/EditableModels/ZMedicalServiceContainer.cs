...
{
    public class ZMedicalServiceContainer : ViewModelBase, IDataErrorInfo
    {
        private readonly DataErrorInfoSupport _dataErrorInfoSupport;
        private readonly IMedicineRepository _repository;
        private readonly Action<int> _changePosition;
        private readonly int _id;
        private Dictionary<int, ZMedicalServiceModel> _cache;
        public List<int> MedicalServiceIds { get; set; }

        public ZMedicalServiceModel MedicalService { get; set; }
        ...
        public ZMedicalServiceContainer(IMedicineRepository repository, int id, Action<int> changePosition)
        {
            _id = id;
        ...
        {
            _cache = new Dictionary<int, ZMedicalServiceModel>();
            var result = _repository.GetZMedicalServiceIdsByMedicalEventId(_id);
            if (result.Success)
            {
        ...
        private void LoadMedicalServiceData(int medicalServiceId)
        {
            if (_cache.ContainsKey(medicalServiceId))
            {
                ...
                {
                    _changePosition(medicalServiceId);
                }
            }
            else
            {
                var medicalServiceResult = _repository.GetZMedicalServiceById(medicalServiceId);
                if (medicalServiceResult.Success)
                {
                    var medicalService = medicalServiceResult.Data;
                    MedicalService = Map.ObjectToObject<ZMedicalServiceModel>(medicalService, medicalService);
