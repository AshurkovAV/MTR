...
{
    public class MedicalEventContainer : ViewModelBase, IDataErrorInfo
    {
        ...
        private Dictionary<int, MedicalEventModel> _cache;
        
        public List<int> MedicalEventIds { get; set; }

        public MedicalEventModel MedicalEvent { get; set; }
        ...
        public MedicalEventContainer(IMedicineRepository repository, int id, Action<int> changePosition)
        {
            _id = id;
        ...
        {
            _cache = new Dictionary<int, MedicalEventModel>();
            var result = _repository.GetMedicalEventIdsByPatientId(_id);
            if (result.Success)
            {
        ...
        private void LoadMedicalEventData(int medicalEventId, bool forced = false)
        {
            ...
            {
                var medicalEventResult = _repository.GetMedicalEventById(medicalEventId);
                if (medicalEventResult.Success)
                {
                    var factMedicalEvent = medicalEventResult.Data;
                    MedicalEvent = Map.ObjectToObject<MedicalEventModel>(factMedicalEvent, factMedicalEvent);
