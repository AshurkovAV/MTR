...
{
    public class EditZMeeViewModel : ClassifierBaseVm<ZFactSank>
    {
        private static readonly ICacheRepository _cacheRepository;
        static EditZMeeViewModel()
        {
            _cacheRepository = Di.I.Resolve<ICacheRepository>();
        }

        public EditZMeeViewModel(ZFactSank classifier)
            : base(classifier)
        {
        }

        public EditZMeeViewModel()
        {
        ...
        [CustomEditor(EditorType = typeof(ComboBoxEdit), ItemSourceType = typeof(F006CacheItemsSource), Value = "Value", DisplayName = "DisplayName")]
        public int? Type
        {
