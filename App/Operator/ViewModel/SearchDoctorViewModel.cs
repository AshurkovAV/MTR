...
namespace Medical.AppLayer.Operator.ViewModel
{
    public class SearchDoctorViewModel : ViewModelBase
    {
        ...
        public SearchDoctorViewModel(ICacheRepository cache, string code)
        {
            _cache = cache;

            _id = _cache.Get(CacheRepository.ShareDoctorToCodeCache).GetBack(code);
        ...
        private void Apply()
        {
            ...
            {
                OkCallback(_cache.Get(CacheRepository.ShareDoctorToCodeCache).Get(SelectedId) as string);
            }
        }
